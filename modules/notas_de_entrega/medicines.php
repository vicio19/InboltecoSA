...
if(isset($_POST['dataidobat'])) {
  ...
  $query = mysqli_query($mysqli2, "SELECT CLICODIGO,CLINOMBRES,CLILIMITE,CLIMONEDA FROM clientes WHERE CLICODIGO='$codigo'")
                                  or die('error '.mysqli_error($mysqli));


  $data = mysqli_fetch_assoc($query);

  $stock   = $data['CLIMONEDA'];
  $unidad = $data['CLILIMITE'];
